...
from OBDModules import OBDConnection
...
def add_info_to_frame(frame, fps, obd_error_shown, obd_connected, obd_connection):
    ...
    if obd_connected and obd_connection and obd_connection.is_connected():
        try:
            # 글로벌 변수 사용
            speed_text, rpm_text, throttle_text, load_text = OBDState.get_state()
        except Exception as e:
    ...
    cv2.putText(frame, f"Speed: {speed_text} RPM: {rpm_text} Throttle: {throttle_text} Load: {load_text}",
                (60, frame.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
