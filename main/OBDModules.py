...
import calculate_driving_score
import obd_state
...
def get_speed(s):
    ...
    if not s.is_null():
        ...
        obd_state.OBDState.update(speed = speed)
        get_Distance(speed)  # 속도가 업데이트될 때마다 거리 계산
...
def get_intake_temp(it):
    ...
    if not it.is_null():
        ...
        obd_state.OBDState.update(intake_temp=intake_temp)
...
def get_rpm(r):
    ...
    if not r.is_null():
        ...
        #print(f"RPM: {rpm}")
        obd_state.OBDState.update(rpm=rpm)
...
def get_load(l):
    ...
    if not l.is_null():
        ...
        obd_state.OBDState.update(load=load)
...
def get_coolant_temp(ct):
    ...
    if not ct.is_null():
        ...
        obd_state.OBDState.update(coolant_temp = coolant_temp)
...
def get_timing_a(ta):
    ...
    if not ta.is_null():
        ...
        obd_state.OBDState.update(timing_advance=timing_advance)
...
def get_throttle_pos(tp):
    ...
    if not tp.is_null():
        ...
        obd_state.OBDState.update(throttle_pos=throttle_pos)
...
def get_short_fuel_trim_1(sft1):
    ...
    if not sft1.is_null():
        ...
        obd_state.OBDState.update(short_fuel_trim_1=short_fuel_trim_1)
...
def get_long_fuel_trim_1(lft1):
    ...
    if not lft1.is_null():
        ...
        obd_state.OBDState.update(long_fuel_trim_1 = long_fuel_trim_1)
...
def get_o2_b1s1(o2b1s1):
    ...
    if not o2b1s1.is_null():
        ...
        obd_state.OBDState.update(o2_b1s1 = o2_b1s1)
...
def get_o2_b1s2(o2b1s2):
    ...
    if not o2b1s2.is_null():
        ...
        obd_state.OBDState.update(o2_b1s2 = o2_b1s2)

def get_save_score():
    global save_score
    save_score = calculate_driving_score.calculate_driving_score(speed, rpm, load, throttle_pos)
    obd_state.OBDState.update(save_score = save_score)

def get_Distance(speed):
    global distance
    distance = calculate_distance.calculate_distance(speed)
    obd_state.OBDState.update(distance = distance)

def get_Acceleration():
    global acceleration
    obd_state.OBDState.update(acceleration = acceleration)

def get_Dceleration():
    global dceleration
    obd_state.OBDState.update(dceleration = dceleration)
